...
class Bus extends promiseEvents.EventEmitter {
    ...
    return transport.init(this).then(() => Promise.all([
      transport.on('bus::nodeInfoInitial', data => { // ~ ACK after SYN-ACK
        ...
        return inflate(data).then(data => {
          ...
          return this.busGraph.addTransport(transport).then(() => false);
        }).then(skip => {
          if (skip) {
            return true;
          }
      ...
      transport.on('disconnect', (reason) => {
        ...
        return this.busGraph.removeTransport(transport.id).then(() => {
          this.transports.delete(transport);
          
          if (this.connectingTransports.size === 0) {
            return this.busGraph.localize();
          }
        }).then(() => {
  ...
  handleBusPacket(packet) {
    ...
    return Promise.all(packet.recipients.map(recpId => {
      ...
      if (recpId === this.id) {
        // defer handling until all other recipients are handled,
        // since we might be receiving a message which invalidates the bus graph
      ...
      if (!path || path.length < 3) {
        ...
        return this.busGraph.once('updated').then(() => {
    ...
    this.responseWaiters.set(requestId, {
      handleResponse: responsePacket => {
        ...
        if (scope === 'nearest') {
          ...
          return deferred.resolve(_.map(responsePackets, 'result'));
        }
      },
